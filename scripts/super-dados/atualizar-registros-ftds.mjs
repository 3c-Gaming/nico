// Le SUPER-JUNHO.csv, consulta /api/tracking/export/utm (fonte sem CPA)
// linha a linha, por DATA + CASA + UTM/PID, e devolve o CSV igual, porem
// com REGISTROS e FTD atualizados. CPAS e CPA ficam intocados — essa fonte
// não tem esse dado.
//
// Uso:
//   node scripts/super-dados/atualizar-registros-ftds.mjs
//   node scripts/super-dados/atualizar-registros-ftds.mjs caminho/entrada.csv caminho/saida.csv
...
const ANO = 2026
const API_BASE = 'https://controlenumeros.vercel.app/api/tracking/export/utm'
const DELAY_MS = 150

// Nome da casa no CSV -> slug esperado pela API.
const CASA_MAP = {
  SuperBet: 'superbet',
  MGMBET: 'mgm',
  MGM: 'mgm',
...
async function main() {
  ...
  for (let i = 0; i < corpo.length; i++) {
    ...
    try {
      ...
      console.log(
        `${progresso} OK -> utm=${utm} date=${dateApi} :: registros=${resultado.registros} ftds=${resultado.ftds}`,
      )
      atualizadas++
    } catch (err) {
