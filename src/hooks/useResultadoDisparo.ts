...
interface UseResultadoDisparoParams {
  ...
  /** Custo adicional somado ao total (ex: SMS de fallback do RCS). */
  custoExtra?: number
}
...
export function useResultadoDisparo({ utmValor, casa, data, entregues, custoPorUnidade, custoExtra }: UseResultadoDisparoParams): UseResultadoDisparoReturn {
  ...
  const custo = (entregues != null ? entregues * (custoPorUnidade ?? CUSTO_POR_ENTREGUE) : 0) + (custoExtra ?? 0)
