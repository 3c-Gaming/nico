...
function DisparoPinadoRow({ disparo, daxxCampanhas, onUnpin, onVerDetalhes, onResultado }: DisparoPinadoRowProps) {
  ...
  // base × custo digitado.
  const [rcsResumo, setRcsResumo] = useState<{ cobravel: number; fallbackEnviados: number } | null>(null)
  useEffect(() => {
    ...
    fetch('/api/rcs/resumo')
      ...
      .then((json) => {
        ...
        setRcsResumo({ cobravel: c?.cobravel ?? c?.enviados ?? 0, fallbackEnviados: c?.fallbackEnviados ?? 0 })
      })
      .catch(() => {})
    return () => { cancel = true }
  ...
  // RCS: cobra por mensagem submetida, menos rejeições por saldo (não cobradas) = "cobravel".
  const entregues =
    disparo.canal === 'rcs' ? (rcsResumo?.cobravel ?? 0)
