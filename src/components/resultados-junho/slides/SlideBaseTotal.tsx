  ...
  return (
    ...
    >
      <SlideItem className="grid grid-cols-2 md:grid-cols-2 gap-4 w-full">
        ...
        <StatTile label="Investido" value={total.custo} prefix="R$ " cor="var(--text-primary)" tamanho="lg" delay={0.1} />
        <StatTile label="Faturamento" value={total.faturamento} prefix="R$ " cor="var(--text-primary)" tamanho="lg" delay={0.2} />
        <StatTile label="Lucro" value={total.lucro} prefix="R$ " cor="var(--success)" tamanho="lg" delay={0.3} />
      </SlideItem>
      ...
      <SlideItem className="w-full flex flex-col gap-2">
        ...
        {top3.map((d, i) => (
          <div
            key={`${d.data}-${d.nome}`}
            className="flex items-center justify-between rounded-lg glass bg-[var(--glass-bg)] border border-[var(--glass-border)] px-4 py-3 text-left"
          >
            ...
            <div className="text-sm font-bold text-[var(--success)] shrink-0 ml-3">{formatarMoeda(d.lucro)}</div>
          </div>
        ))}
      </SlideItem>
    </SlideShell>
